...
export default class Post extends Component {
    ...
    handleChanges = (event) => {
        this.setState({...this.stat, [event.target.name]: event.target.value });
    }
    ...
    render() {
        ...
        return (
            <div>
                <p>{id} {title} {description} {timetomaster} {source} {moment(startlearningdate).format('DD.MM.YYYY')} {inprogress} {moment(finishlearningdate).format('DD.MM.YYYY')} {timespent} <button onClick={this.deleteRow}>X</button><button onClick={this.editRow}>edit</button></p>
                <h4>Edit post</h4>
